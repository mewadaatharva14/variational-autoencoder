...
class VAETrainer:
    ...
    def _save_reconstruction_grid(self, real: torch.Tensor, epoch: int) -> None:
        ...
        comparison = torch.cat([real[:8].to(self.device), recon], dim=0)
    ...
    def _save_checkpoint(self, epoch: int, total_loss: float) -> None:
        ...
        torch.save(ckpt, os.path.join(self.checkpoint_dir, f"checkpoint_epoch_{epoch:03d}.pth"))
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch_idx, (imgs, _) in enumerate(pbar):
                ...
                # ── Loss ──────────────────────────────────────────────
                total_loss, recon_loss, kl_loss = self.model.loss(imgs, x_recon, mu, log_var)
        ...
        torchvision.utils.save_image(
            torch.cat(
                [first_batch[:8].to(self.device), self.model(first_batch[:8].to(self.device))[0]]
            ),
